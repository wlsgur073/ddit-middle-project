...
@WebServlet("/ClipJoCal.do")
public class ClipJoCal extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(memVO != null) {
			...
			List<ScheduleVO> clipCal = scheduleService.getClipCal(hrVO);
			
			req.setAttribute("clipCal", clipCal);
			
			System.out.println(clipCal == null);
			
			req.getRequestDispatcher("/mainJsp/myClipCalendar.jsp").forward(req, resp);
		}else {
			System.out.println("ClipJoCal 에서 로그인멤버를 찾을 수 없음");
			IScheduleService scheduleService = ScheduleServiceImpl.getInstance();
